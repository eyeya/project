                ...
                <div class="card-body">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            //%m เป็นเดือนแบบ เลข %M แบบหนังสือ

                            $sql = "SELECT DATE_FORMAT(booking.datecreate, '%Y-%m') AS month_name,
                                SUM(course.price) as total_price,
                                COUNT(booking.id_course) AS total_booking";
                            $sql .= " FROM booking INNER JOIN course ON course.id = booking.id_course";
                            if ($search) {
                                if ($s2 == '') {
                                    $sql .= " WHERE DATE_FORMAT(booking.datecreate, '%m') = '$s1' "; //ค้นหาเฉพาะเดือน
                                } else {
                                    $sql .= " WHERE DATE_FORMAT(booking.datecreate, '%m') BETWEEN '$s1' AND '$s2'"; //ค้นหาตั้งแต่เดือน... ถึงเดือน...
                                }
                            }
                            ...
                            ?>
                                <tr>
                                    <td><?php echo $i ?></td>
                                    <td><?php echo DBThaiLongDate1($row['month_name']) ?></a></td>
...
<!-- <?php
        include 'config.php';
        $chart = "SELECT DATE_FORMAT(booking.datecreate, '%M-%Y') AS month_name, SUM(course.price) as total_mounth FROM booking INNER JOIN course ON course.id = booking.id_course GROUP BY DATE_FORMAT(booking.datecreate, '%M-%Y') ORDER BY DATE_FORMAT(booking.datecreate, '%m-%Y') ASC;";
        $r = mysqli_query($conn, $chart);
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="col-12">
                        <canvas id="myChart" style="max-width:1000px;max-height:450px;margin-bottom:25px;"></canvas>
                    </div>
                </div>
            </div>
        </div>
